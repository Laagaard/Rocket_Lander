...
float kp_body_x_rot = .5;//0.25;//.5;
float kp_body_y_rot = .5;//0.25;//.5;
float kd_body_x_rot = 2;//0;
float kd_body_y_rot = 2;//0;
...
void terminalGuidance(){
  //terminal(); //lol
  guidance();
  if(micros() - lastPIDTime > 50000){ //1000000/20
    lastPIDTime = micros();
    updatePID();
...
void updatePID(){
    ...
    float dT = 0.05;//50/1000; //TBR: idealized
    ...
    float d_body_x_error = (current_body_x_error - last_body_x_error); //divide by zero lol
    float d_body_y_error = (current_body_y_error - last_body_y_error);
