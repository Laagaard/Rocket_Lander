...
unsigned int telemetryRate = 2; //hz //TBR: may be a ground station problem, data wont report over 10 hz
...
int expected_motor_burn_time = 4000; //ms
String dataString = "";
...
float kp_body_x_rot = 0.25;//.5;
float kp_body_y_rot = 0.25;//.5;
float kd_body_x_rot = 0;//0;
float kd_body_y_rot = 0;//0;
float ki_body_x_rot = 0;//.00002;
float ki_body_y_rot = 0;//.00002;
int ignitionDelay = 750;//ms
...
void loop() {
  ...
  attitudeControlTestUpdateData(globalTimer, machineState);
...
void unpoweredDescent(){
  ...
  if(false){          //TBR: add detection of motor fire start here 
    releaseLegs();
    lastPIDTime = millis(); //must be updated before updatePID is called for the first time, 
...
void updatePID(){
  ...
  float realdT = (millis() - lastPIDTime)/1000; //these jumps are super small gives bad control accuracy, change method for fixed control rate
                                            //TBR: PROGRAM TIME SPIKES LEADING TO HIGH DT, HIGHER INTEGRAL TERM??
  lastPIDTime = millis();

  float dT = 50/1000; //TBR: idealized
  ...
  float d_body_x_error = (current_body_x_error - last_body_x_error);//realdT; //divide by zero lol
  float d_body_y_error = (current_body_y_error - last_body_y_error);//realdT;
...
void attitudeControlTestUpdateData(uint32_t datatimer, int machineState){
  ...
  altDataString += ",";
  if(continuityState2 == 1){altDataString += "1,";}
  else{altDataString += "0,";}
