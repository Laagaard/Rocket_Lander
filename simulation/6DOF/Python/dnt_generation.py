...
from monte_carlo_simulation import CSV_output_dir

timestep_current_lower_bound = 0.1 # [s] lower time bound of current discretization
...
# Loop until the lower time bound is greater than the optimal trajectory flight time
while (timestep_current_lower_bound < optimal_df["Time"].iloc[-1]):
    ...
    for filename in os.listdir(CSV_output_dir):
        ...
        if (df["landing_zone_flag"][0] == 0): # the trajectory impacts outside the landing zone
            continue
