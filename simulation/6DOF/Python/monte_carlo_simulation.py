...
import stat
import sys
from optimal_trajectory import trajectory_csv_header
...
if __name__ == "__main__":
    ...
    monte_carlo_fig = plt.figure()
    monte_carlo_ax = monte_carlo_fig.add_subplot()
    monte_carlo_ax.plot(0, 0, 'k.', label="Launch Site") # plot launch site (i.e., inertial CS origin)
    monte_carlo_ax.plot(landing_zone_x, landing_zone_y, color='r', marker="+") # plot center of the landing zone
    monte_carlo_ax.plot(x_lz_interval, landing_zone_upper_edge, color='r', label="Landing Zone") # plot upper semi-circle of the landing zone
    monte_carlo_ax.plot(x_lz_interval, landing_zone_lower_edge, color='r') # plot lower semi-circle of the landing zone
    monte_carlo_ax.add_patch(landing_zone_patch) # plot landing zone patch (for some reason, must be plotted for `contains_point` method to work properly)
    monte_carlo_ax.set_xlabel("X - East [m]")
    monte_carlo_ax.set_ylabel("Y - North [m]")
    monte_carlo_ax.axis('equal') # set axis limits equivalent
    monte_carlo_ax.grid(which="major", axis="both")
    ...
    num_of_successes = 0 # number of trajectories that impact within the landing zone
    ...
    for traj in range(num_of_trajectories):
        ...
        if (landing_zone_patch.contains_point(point=monte_carlo_ax.transData.transform(values=(test_flight.x_impact, test_flight.y_impact)))): # trajectory's impact location is within the landing zone
            monte_carlo_ax.plot(test_flight.x(solution_time), test_flight.y(solution_time), 'b.')
            landing_zone_flag = 1 # to be written to the CSV file
            num_of_successes += 1
        else: # trajectory's impact location is not within the landing zone
            # monte_carlo_ax.plot(test_flight.x(solution_time), test_flight.y(solution_time), 'r.')
        ...
        for time_step in solution_time:
            time_step_data = [time_step, test_flight.x(time_step), test_flight.y(time_step), test_flight.z(time_step), test_flight.vx(time_step), test_flight.vy(time_step), landing_zone_flag]
            writer.writerow(time_step_data)
        output_file.close()

    success_rate = num_of_successes/num_of_trajectories # fraction of trajectories that impacted within the landing zone
    monte_carlo_ax.set_title(f"Monte Carlo Simulation Results \n(Trajectory Simulations: {num_of_trajectories} , Success Rate: {success_rate})")
    plt.show()

    print(f"---------- SIMULATION RESULTS ----------")
    print(f"Trajectories Simulated: {num_of_trajectories}")
    print(f"Successes: {num_of_successes}")
